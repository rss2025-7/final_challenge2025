...
class PathPlan(Node):
    ...
    def dilate_obstacles(self, robot_radius_m):
        ...
        self.get_logger().info("dilating")
        selem = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (radius_px, radius_px))

        # d) Morphological dilation
        # dilated_map = dilation(binary_map, selem)
        dilated_map = cv2.dilate(binary_map, selem, iterations = 1)
